...
def index(request):
    ...
    return render(request,'lh_goods/index.html',context)
...
def list(request,tid,pindex,sort):
    ...
    context={'title':typeinfo.ttitle,
             ...
             'sort':sort,
             'news':news}
    return render(request,'lh_goods/list.html',context)
def detail(request,id):
    ...
    context={'title':goods.gtype.ttitle,
             ...
             'news':news,
             'id':id}
    response=render(request,'lh_goods/detail.html',context)

    #记录最近浏览，在用户中心使用
    goods_ids=request.COOKIES.get('goods_ids','')
    goods_id='%d'%goods.id
    if goods_ids!='':#判断是否有最近浏览记录，如果有则继续判断
